...
clone_repo() {
    # check if CythonX, startup, plugins folders exist
    cd $DIR
    if [ -d $DIR ]; then
        if [ -d $DIR/.git ]; then
            echo -e "Updating Megatron ${BRANCH}... "
            cd $DIR
            git pull
        ...
        mkdir -p $DIR
        echo -e "Cloning Megatron ${BRANCH}... "
...
misc_install() {
    ...
    if [ $OKTETO_TOKEN ]; then
        ...
        if [ -d $DIR/vcbot ]; then
            ...
            git clone https://github.com/ToxygenX/VcBot $DIR/vcbot
        fi
...
main() {
    echo -e "Starting ToxygenX Setup..."
    if [ -d "CythonX" ] && [ -d "resources" ] && [ -d "plugins" ]; then
        DIR=$CURRENT_DIR
    fi
    if [ -f $ENV_FILE_PATH ]
    then
